...
def compute_clip_combo(images, seq_words, clip_model, gen_score, fine_tuned=False, img_encoder=None, txt_encoder=None):
  sim_scores = []
  seq_i = 0
  for group in images:
    #print(group)
    if None in group:
      ...
      with torch.no_grad():
        if fine_tuned:
            image_features = img_encoder(image_input).float()
            text_features = txt_encoder(text_tokens).float()
...
def train(opt):
    ...
    model_path = "../../scp_files/vigorous-germain/models"
    model_path1 = "../../scp_files/youthful-lewin/models"
    ...
    image_encoder1 = CLIPVisionBuilder(descriptor='openai/clip-vit-base-patch32').build()
    image_encoder1.load_state_dict(torch.load(model_path1 + "/clip-vision/model.pt"))
    text_encoder1 = CLIPTextBuilder(descriptor='openai/clip-vit-base-patch32').build()
    text_encoder1.load_state_dict(torch.load(model_path1 + "/clip-text/model.pt"))
    image_encoder1 = image_encoder.to("cuda")
    text_encoder1 = text_encoder.to("cuda")
    ...
    disc.load_state_dict(torch.load(os.path.join('data/save/AREL', 'disc-model.pth')))
    disc.cuda()
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            clip_gen_score = compute_clip_combo(images, seq_words, clip_model, gen_score)

            clip_gen_score1 = compute_clip_combo(images, seq_words, clip_model, gen_score, fine_tuned=finetuned, img_encoder=image_encoder, txt_encoder=text_encoder)
            
            clip_gen_score2 = compute_clip_combo(images, seq_words, clip_model, gen_score, fine_tuned=finetuned, img_encoder=image_encoder1, txt_encoder=text_encoder1)

            #print("hello?")
            #print("inter")

            disc_test = {
                'pretrained': clip_gen_score,
                'ft-200': clip_gen_score1,
                'ft-1000': clip_gen_score2,
                'disc': gen_score.cpu(),
            }
            
            with open("disc_test.pkl", 'wb') as d:
                pickle.dump(disc_test, d)
